...
2. 재고 판정
   ...
   - **재고 충분** (`stockQuantity >= quantity`): 즉시 재고를 `quantity` 만큼 차감하고 주문 상태를 `CONFIRMED` 로 전환
...
> **주의(재고 일부 활용 여부)**: PRD 는 "재고가 부족한 경우 생산 라인에 자동 등록"만 명시하고, 기존 재고 일부를 우선 차감할지 여부는 명시하지 않는다. 구현 시 정책을 확정해야 하며, 기본 정책은 다음과 같이 제안한다.
> - 재고 부족 시에도 기존 재고는 유지한 채 부족분(`shortageQuantity`)만큼만 생산하고, 생산 완료 시 "생산량 + 기존 재고"에서 주문 수량을 차감한다.
> - 이 정책은 재고가 다른 대기 주문에 동시에 배정되지 않도록 하는 별도 예약(reservation) 로직이 필요할 수 있음에 유의한다 (동시에 여러 주문이 PRODUCING 으로 전환되는 경우 재고 이중 사용 방지). 세부 동시성 정책은 구현 단계에서 별도로 확정한다.
