...
1. 작업 지시에 명시된 `phase{N}.md`를 읽는다. 그 파일에 적힌 요구사항만을 근거로 gmock 테스트를 작성한다. phase{N}.md에 "참고 PoC" 절이 있다면 참고로 훑어볼 수 있으나, 테스트는 어디까지나 phase{N}.md의 요구사항을 근거로 작성하고 PoC의 구현 세부사항에 맞춰 테스트를 짜지 않는다.
