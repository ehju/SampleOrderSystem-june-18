...
- `poc/` 폴더가 있다고 자동으로 참조하지 않는다. 사용자가 특정 PoC 코드를 참조하라고 명시적으로 지시한 경우에만 확인하고 참조한다.
- PoC는 동작 방식과 구조를 파악하기 위한 참고 자료이며, 그대로 복사하지 않고 이 문서의 네이밍/서식 규칙에 맞게 재작성한다.
- PoC와 요구사항(docs 폴더 스펙)이 다르면 요구사항을 따른다.
